              ...
              children: [
                ...
                Center(
                  child: Padding(
                    ...
                    child: GlassPiggyBank(currentCoins: currentCoins, fallingCoin: fallingCoin),
                  ),
                ),
...
class PiggyBankBackgroundPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void drawBokeh(Offset center, double radius, Color color) {
      canvas.drawCircle(center, radius, Paint()..color = color..maskFilter = const MaskFilter.blur(BlurStyle.normal, 20));
    }
    drawBokeh(Offset(size.width * 0.2, size.height * 0.3), size.width * 0.3, Colors.white.withValues(alpha: 0.5));
    drawBokeh(Offset(size.width * 0.8, size.height * 0.4), size.width * 0.4, const Color(0xFFFFFACD).withValues(alpha: 0.3));
    ...
    // 奥の丘
    canvas.drawOval(
      Rect.fromCenter(center: Offset(size.width * 0.2, size.height * 0.9), width: size.width * 1.5, height: size.height * 0.6),
      hillPaint1,
    );
    // 手前の丘
    canvas.drawOval(
      Rect.fromCenter(center: Offset(size.width * 0.8, size.height * 0.95), width: size.width * 1.5, height: size.height * 0.5),
      hillPaint2,
    );
    ...
    void drawFlower(Offset pos, double sizeParam, Color petalColor) {
      ...
      // 影
      canvas.drawCircle(pos + const Offset(0, 2), sizeParam * 1.2, Paint()..color = Colors.black.withValues(alpha: 0.05)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 2));
      ...
      // 中心
      canvas.drawCircle(pos, sizeParam * 0.45, Paint()..color = const Color(0xFFFFD54F));
    }

    drawFlower(Offset(size.width * 0.22, size.height * 0.88), 10, const Color(0xFFFFF2ED));
    drawFlower(Offset(size.width * 0.8, size.height * 0.92), 12, const Color(0xFFFFE4E1));
    drawFlower(Offset(size.width * 0.1, size.height * 0.8), 8, const Color(0xFFE0FFFF));

    // 5. 蝶々（♡）
    void drawButterflyHeart(Offset pos, double sizeParam, double rotation, Color color) {
      canvas.save();
      canvas.translate(pos.dx, pos.dy);
      ...
      path.cubicTo(-sizeParam, -sizeParam * 0.5, -sizeParam * 0.2, -sizeParam, 0, -sizeParam * 0.3);
      path.cubicTo(sizeParam * 0.2, -sizeParam, sizeParam, -sizeParam * 0.5, 0, sizeParam * 0.3);
      canvas.drawPath(path, paint);
      
      canvas.restore();
    }

    drawButterflyHeart(Offset(size.width * 0.15, size.height * 0.5), 10, -0.2, const Color(0xFFFFB6C1).withValues(alpha: 0.9));
    drawButterflyHeart(Offset(size.width * 0.82, size.height * 0.3), 14, 0.3, const Color(0xFFFF69B4).withValues(alpha: 0.8));
    drawButterflyHeart(Offset(size.width * 0.75, size.height * 0.55), 8, 0.1, const Color(0xFFFFC0CB).withValues(alpha: 0.9));
    drawButterflyHeart(Offset(size.width * 0.3, size.height * 0.2), 12, -0.4, const Color(0xFFFFA07A).withValues(alpha: 0.8));
...
class GlassPiggyBank extends StatelessWidget {
  final int currentCoins;
  final Widget? fallingCoin;
  const GlassPiggyBank({super.key, required this.currentCoins, this.fallingCoin});
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < displayCoins; i++) {
      ...
      final double cx = bellyCenterX + math.cos(angle) * (bellyWidth / 2 * 0.85) * r; 
      final double cy = bellyCenterY + (bellyHeight * 0.1) + (math.sin(angle) * (bellyHeight / 2 * 0.75) * r) - (i * 1.5); 
...
class PiggyBankBellyClipper extends CustomClipper<Path> {
  ...
  final double width;
  final double height;
  PiggyBankBellyClipper({required this.center, required this.width, required this.height});
...
class GlassPiggyBankBackPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void drawBackLeg(Offset legCenter) {
      final legRect = Rect.fromCenter(center: legCenter, width: width * 0.12, height: height * 0.25);
      final legRRect = RRect.fromRectAndRadius(legRect, const Radius.circular(15));
      
      // 暗めのガラス影
      canvas.drawRRect(legRRect, Paint()
      ...
      canvas.drawRRect(legRRect, Paint()
        ..shader = LinearGradient(
          ...
          colors: [Colors.white.withValues(alpha: 0.15), Colors.white.withValues(alpha: 0.4)],
    ...
    // 投入口の奥側（暗いスリット線）
    final slotCenter = Offset(center.dx + width * 0.05, center.dy - height * 0.48);
    final slotWidth = width * 0.22;
    final slotDarkPath = Path();
    slotDarkPath.moveTo(slotCenter.dx - slotWidth * 0.5, slotCenter.dy);
    slotDarkPath.quadraticBezierTo(slotCenter.dx, slotCenter.dy - height * 0.02, slotCenter.dx + slotWidth * 0.5, slotCenter.dy + height * 0.015);
    
    canvas.drawPath(slotDarkPath, Paint()
...
class GlassPiggyBankFrontPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawOval(
      Rect.fromCenter(center: center, width: width * 0.96, height: height * 0.96),
      Paint()
    ...
    windowHighlight.quadraticBezierTo(center.dx, center.dy - height * 0.48, center.dx + width * 0.25, center.dy - height * 0.3);
    windowHighlight.lineTo(center.dx + width * 0.2, center.dy - height * 0.25);
    windowHighlight.quadraticBezierTo(center.dx, center.dy - height * 0.4, center.dx - width * 0.25, center.dy - height * 0.33);
    windowHighlight.close();
    
    canvas.drawPath(windowHighlight, Paint()
      ..shader = LinearGradient(
        begin: Alignment.topLeft,
        end: Alignment.bottomRight,
        colors: [Colors.white.withValues(alpha: 0.3), Colors.transparent], // 透明度を下げて柔らかく
      ).createShader(windowHighlight.getBounds())
    );

    // 3. 手前の足
    void drawFrontLeg(Offset legCenter) {
      final legRect = Rect.fromCenter(center: legCenter, width: width * 0.14, height: height * 0.28);
      final legRRect = RRect.fromRectAndRadius(legRect, const Radius.circular(15));
    ...
    final snoutCenter = Offset(center.dx - width * 0.4, center.dy + height * 0.05);
    final snoutWidth = width * 0.22;
    final snoutHeight = height * 0.32;
    final snoutRect = Rect.fromCenter(center: snoutCenter, width: snoutWidth, height: snoutHeight);
    
    // 白いプラスチック/ガラスの土台フチ（細くする）
    canvas.drawOval(snoutRect.inflate(2), Paint()
    ...
    // 少し透明感のあるリアルなピンクの鼻
    canvas.drawOval(snoutRect, Paint()
      ..shader = RadialGradient(
    ...
    // 鼻の穴
    final leftNostril = Rect.fromCenter(center: Offset(snoutCenter.dx - snoutWidth * 0.12, snoutCenter.dy), width: snoutWidth * 0.12, height: snoutHeight * 0.25);
    final rightNostril = Rect.fromCenter(center: Offset(snoutCenter.dx + snoutWidth * 0.12, snoutCenter.dy), width: snoutWidth * 0.12, height: snoutHeight * 0.25);
    final nostrilPaint = Paint()..color = const Color(0xFFAD1457).withValues(alpha: 0.6);
    canvas.drawOval(leftNostril, nostrilPaint);
    canvas.drawOval(rightNostril, nostrilPaint);

    // 5. 目 (Eyes)
    void drawEye(Offset eyeCenter) {
      final eyeRect = Rect.fromCenter(center: eyeCenter, width: width * 0.045, height: height * 0.12);
      // ツヤのある黒目
      canvas.drawOval(eyeRect, Paint()..color = const Color(0xFF2C2C2C));
      // 上の強いハイライト
      canvas.drawCircle(Offset(eyeCenter.dx - 2, eyeCenter.dy - 4), 2.5, Paint()..color = Colors.white);
      // 下の照り返し
      canvas.drawCircle(Offset(eyeCenter.dx + 1.5, eyeCenter.dy + 3), 1.2, Paint()..color = Colors.white.withValues(alpha: 0.8));
    }
    ...
    backEarPath.quadraticBezierTo(center.dx - width * 0.05, center.dy - height * 0.55, center.dx + width * 0.1, center.dy - height * 0.6);
    backEarPath.quadraticBezierTo(center.dx + width * 0.2, center.dy - height * 0.5, center.dx + width * 0.15, center.dy - height * 0.4);
    backEarPath.close();
    ...
    frontEarPath.quadraticBezierTo(center.dx - width * 0.45, center.dy - height * 0.5, center.dx - width * 0.28, center.dy - height * 0.55);
    frontEarPath.quadraticBezierTo(center.dx - width * 0.1, center.dy - height * 0.45, center.dx - width * 0.15, center.dy - height * 0.35);
    frontEarPath.close();
    ...
    rimPath.quadraticBezierTo(slotCenter.dx, slotCenter.dy - height * 0.02 + 2, slotCenter.dx + slotWidth * 0.48, slotCenter.dy + height * 0.015 + 2);
    ...
    tailPath.quadraticBezierTo(center.dx + width * 0.65, center.dy + height * 0.05, center.dx + width * 0.6, center.dy - height * 0.05);
    tailPath.quadraticBezierTo(center.dx + width * 0.5, center.dy - height * 0.1, center.dx + width * 0.55, center.dy + height * 0.05);
...
class _Coin3DPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (!isFront) {
      ...
      final edgeColor = hsl.withLightness(math.max(0.0, hsl.lightness - 0.15)).toColor();
    ...
    final faceGradient = LinearGradient(
      ...
      colors: [
        HSLColor.fromColor(baseColor).withLightness(math.min(1.0, HSLColor.fromColor(baseColor).lightness + 0.15)).toColor(),
        baseColor,
        HSLColor.fromColor(baseColor).withLightness(math.max(0.0, HSLColor.fromColor(baseColor).lightness - 0.1)).toColor(),
      ],
    ...
    textPainterShadow.paint(canvas, center - Offset(textPainterShadow.width / 2 - 1, textPainterShadow.height / 2 - 1));
    ...
    final textPainterFront = TextPainter(
      ...
      text: TextSpan(
        ...
        style: TextStyle(
          ...
          fontFamily: icon.fontFamily,
          package: icon.fontPackage,
          color: HSLColor.fromColor(baseColor).withLightness(math.min(1.0, HSLColor.fromColor(baseColor).lightness + 0.15)).toColor(), 
        ),
      ),
    );
    textPainterFront.layout();
    textPainterFront.paint(canvas, center - Offset(textPainterFront.width / 2 + 1, textPainterFront.height / 2 + 1));
  }
...
class PiggyBankBackPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawOval(
      Rect.fromCenter(center: Offset(center.dx, size.height + 5), width: radius * 1.5, height: 40),
      shadowPaint,
    );
...
class PiggyBankFrontPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    windowHighlight.quadraticBezierTo(center.dx - radius * 0.35, center.dy - radius * 0.85, center.dx + radius * 0.2, center.dy - radius * 0.8);
    windowHighlight.lineTo(center.dx + radius * 0.3, center.dy - radius * 0.6);
    windowHighlight.quadraticBezierTo(center.dx - radius * 0.1, center.dy - radius * 0.65, center.dx - radius * 0.45, center.dy - radius * 0.1);
    windowHighlight.close();
    ...
    // left eye
    canvas.drawArc(
      Rect.fromCenter(center: Offset(size.width * 0.32, size.height * 0.42), width: 18, height: 16),
      math.pi * 1.1, math.pi * 0.8, false, eyePaint
    );
    // right eye
    canvas.drawArc(
      Rect.fromCenter(center: Offset(size.width * 0.58, size.height * 0.45), width: 18, height: 16),
      math.pi * 1.1, math.pi * 0.8, false, eyePaint
    );
  }
  ...
  void _drawSnout(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(snoutCenter + const Offset(-4, 8), snoutRadius * 0.9, snoutShadow);

    // Soft glass gradient fill (no hard strokes)
    final snoutPaint = Paint()
      ..shader = RadialGradient(
        colors: [Colors.white.withValues(alpha: 0.2), Colors.white.withValues(alpha: 0.7), Colors.white.withValues(alpha: 0.95)],
    ...
    final snoutInner = Paint()
      ..shader = RadialGradient(
        colors: [Colors.transparent, Colors.black.withValues(alpha: 0.15)],
        stops: const [0.7, 1.0],
      ).createShader(Rect.fromCircle(center: snoutCenter, radius: snoutRadius - 2));
    ...
    // Left hole
    final leftHole = Rect.fromCenter(center: snoutCenter + const Offset(-12, 2), width: 10, height: 20);
    canvas.drawOval(leftHole.translate(1, 1), holeHighlight);
    canvas.drawOval(leftHole, holePaint);
    
    // Right hole
    final rightHole = Rect.fromCenter(center: snoutCenter + const Offset(12, -2), width: 10, height: 20);
  ...
  void _drawLeg(Canvas canvas, Offset pos, double size) {
    ...
    final paint = Paint()
      ..shader = RadialGradient(
        colors: [Colors.white.withValues(alpha: 0.2), Colors.white.withValues(alpha: 0.7), Colors.white.withValues(alpha: 0.9)],
        stops: const [0.3, 0.8, 1.0],
...
class PiggyBankSlotPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final slotDark = Paint()
      ...
      ..style = PaintingStyle.fill;
    final slotRect = Rect.fromCenter(center: slotCenter + const Offset(0, -2), width: 55, height: 14);
    ...
    final hlPaint = Paint()
      ...
      ..style = PaintingStyle.stroke
      ..strokeWidth = 3;
    canvas.drawArc(Rect.fromCenter(center: slotCenter + const Offset(0, 2), width: 60, height: 16), 0, math.pi, false, hlPaint);
  }
